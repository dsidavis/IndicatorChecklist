...
#cat("observeEvent\n")                            
#output$foo = renderText("hi there")    
  observeEvent(input$uncheckBtn,  {
            session$sendCustomMessage(type = 'testmessage', message = 'Thank you for clicking')
                        })

#                         updateCheckboxGroupInput(session, inputId = "issuevect", choices = integrateds, selected = NULL)
#                         updateCheckboxGroupInput(session, inputId = "issuevect", choices = integrateds, selected = NULL)  
# observe(if(input$uncheck) {
#     cat("unchecking\n")
#
#           updateCheckboxGroupInput(session, inputId = "issuevect", choices = integrateds, selected = NULL)            
#         })
  ...
  output$indicatorResults <- renderTable(
    createchecklist(indvect2, makeissuevect(input$issuevect, integratedonly = TRUE), issueindmat2, input$requireds, input$excludeds))

#  output$exOut <- renderText(paste(input$excludeds, collapse = ", "))
})
